...
## This repo (WHW itself)

- Runtime: Node.js ≥ 22.13, **zero runtime dependencies** — never add one
  without an ADR.
- Tests: `npm test` (`node --test "tests/**/*.test.js"`); CLI: `node ./bin/whw.js …`.
- Layout: `src/` (ESM + JSDoc + SPDX headers), `sql/`, `roles/`, `skills/`,
  `templates/`, `docs/why|how|what`, `docs/adr/`, `planning/`, `examples/`, `tests/`.
- Program 001 charter: `docs/adr/0008-program-001.md` (waves 001–006, no 007
  without a new charter).
- Conventions: `docs/how/conventions.md`; roles in `roles/`; skills in `skills/`.
...
- Never name private origins: this repo cites prior production experience only
  as "a long-running production project by the author" (ADR 0007).
