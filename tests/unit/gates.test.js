...
describe('gate runner', () => {
  ...
  it('maint-audit skips non-git repos', async (t) => {
    const gateMod = (await import('../../src/gates/builtin/maint-audit.js'));
    const emptyCtx = makeCtx(makeTmp());
    const emptyDb = openDb(emptyCtx.paths.state);
    t.after(() => closeDb(emptyDb));
    const skipGit = await gateMod.run(emptyCtx, emptyDb);
